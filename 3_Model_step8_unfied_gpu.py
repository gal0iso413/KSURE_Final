"""
XGBoost Risk Prediction Model - Step 8 (GPU): Hyperparameter Tuning (Unified Multiclass)
=====================================================================================

GPU-enabled variant of Step 8 for unified multiclass tuning using XGBoost.
...
- This file mirrors `3_Model_step8_unfied.py` but runs XGBoost with GPU
...
# -----------------------------------------------------------------------------
# Optuna objective (Unified Multiclass, GPU)
# -----------------------------------------------------------------------------
...
def tune_unified_multiclass(X: pd.DataFrame, y: pd.DataFrame, target_cols, precomputed_folds: dict, n_trials: int = TRIALS):
    study = optuna.create_study(
        ...
        direction='maximize',
        study_name='unified_multiclass_study_gpu'
    )
...
def main():
    ...
    print("🚀 STEP 8 (GPU): Hyperparameter Tuning with Optuna (Unified Multiclass)")
    print("=" * 70)
    ...
    print("   🔧 Tuning unified multiclass model (GPU; class weights; argmax)…")
    ...
    with open(os.path.join(RESULTS_DIR, 'step8_optuna_summary_gpu_unified.json'), 'w', encoding='utf-8') as f:
    ...
    with open(os.path.join(RESULTS_DIR, 'step8_best_params_unified_gpu.json'), 'w', encoding='utf-8') as f:
        json.dump(compact, f, indent=2, ensure_ascii=False)

    print("\n🎉 STEP 8 (GPU) TUNING COMPLETED!")
    print("=" * 70)
    print(f"✅ Results saved: {os.path.join(RESULTS_DIR, 'step8_optuna_summary_gpu_unified.json')}")
    print(f"✅ Best params saved: {os.path.join(RESULTS_DIR, 'step8_best_params_unified_gpu.json')}")
